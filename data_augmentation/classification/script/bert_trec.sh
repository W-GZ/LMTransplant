...
for augmenter in $augmentator_list; do
    ...
    # BERT-base-uncased
    # python $SRC/bert_classifier.py --cuda 2 --dataset $DATASET --exp_id ${i} --augmenter $augmenter --seednum ${SEEDNUM} --cache $CACHE > $RAWDATADIR/train_subsample_0${SEEDNUM}_${augmenter}_BERT-base-uncased.log
    ...
    # ModernBERT-large
    # python $SRC/bert_classifier.py --cuda 2 --dataset $DATASET --exp_id ${i} --augmenter $augmenter --seednum ${SEEDNUM} --cache $CACHE > $RAWDATADIR/train_subsample_0${SEEDNUM}_${augmenter}_ModernBERT-large.log --model_type ModernBERT-large
