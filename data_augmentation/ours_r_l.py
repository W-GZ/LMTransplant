...
class OurMethod():
    ...
    def construct_messages_for_bidirectional_completion_r_l(self, sample=None, text_type=None, label_type=None):
        single_turn_prompt = (
            f"Given the original {text_type}, generate a succeeding sentence and a preceding sentence as follows:\n"
            ...
            f"New Text: Place the succeeding sentence behind the original {text_type} to form a new text.\n"
            f"Preceding Sentence: Create a sentence that can naturally precede the new text, making the transition smooth and logical.\n\n"

            f"The original {text_type} is: {sample}\n\n"

            f"Now please return the generated succeeding sentence and preceding sentence in the following format:\n"
    ...
    for item in tqdm(data, desc='Processing', unit='item'):
        if dataset_name in ["SNIPS", "SST2", "TREC"]:
            ...
            for temp in range(augmentation_num):
                ...
                messages_for_bidirectional_completion = ourmethod.construct_messages_for_bidirectional_completion_r_l(sample=input_text, text_type=text_type, label_type=label_type)
