...
class OurMethod():
    ...
    ######### Bidirectional Text Completion #########
    def construct_messages_for_bidirectional_completion_l_r(self, sample=None, text_type=None, label_type=None):
        single_turn_prompt = (
            f"Given the original {text_type}, generate a preceding sentence and a succeeding sentence as follows:\n"
            f"Preceding Sentence: Create a sentence that can naturally precede the original {text_type}, making the transition smooth and logical.\n"
            f"New Text: Place the preceding sentence in front of the original {text_type} to form a new text.\n"
            f"Succeeding Sentence: Generate a sentence that can naturally follow the new text, ensuring a smooth transition and logical continuation.\n\n"

            f"The original {text_type} is: {sample}\n\n"

            f"Now please return the generated preceding sentence and succeeding sentence in the following format:\n"
            f"Preceding Sentence: [The generated preceding sentence]\n"
    ...
    def construct_messages_for_infilling(self, pre_sentence, post_sentence, original_text, text_type=None, label_type=None, original_label=None, label_set=None):
        single_turn_prompt = (
            f"You are provided with three pieces of text:\n"
            f"1. Preceding Sentence: {pre_sentence}\n"
            f"2. Original {text_type}: {original_text}\n"
            f"3. Succeeding Sentence: {post_sentence}\n\n"
            ...
            f"1. Fits naturally between the preceding sentence and succeeding sentence, maintaining logical flow and coherence.\n"
            f"2. Similar in text length, format (sentence pair, etc.), and language style to the original {text_type}.\n"
            # f"3. Similar {label_type} to the original {text_type}, which is one of {self._label_enum_str(label_set)}.\n\n"
            ...
            f"Preceding Sentence: [The provided preceding sentence]\n"
    ...
    for item in tqdm(data, desc='Processing', unit='item'):
        if dataset_name in ["SNIPS", "SST2", "TREC"]:
            ...
            for temp in range(augmentation_num):
                ...
                messages_for_bidirectional_completion = ourmethod.construct_messages_for_bidirectional_completion_l_r(sample=input_text, text_type=text_type, label_type=label_type)
                bidirectional_completion = ourmethod.client.completions_create(messages=messages_for_bidirectional_completion, temperature=temperature)
                dict_1 = split_text_by_subsentences(bidirectional_completion, ["Preceding Sentence", "Original Text", "Succeeding Sentence"])

                if "Preceding Sentence" not in dict_1 or "Succeeding Sentence" not in dict_1:
                    continue
                else:
                    pre_sentence = dict_1['Preceding Sentence']
                    post_sentence = dict_1['Succeeding Sentence']

                    messages_for_infilling = ourmethod.construct_messages_for_infilling(pre_sentence, post_sentence, input_text, text_type=text_type, label_type=label_type, original_label=input_label)
                    text_infilling = ourmethod.client.completions_create(messages=messages_for_infilling, temperature=temperature)
                    dict_2 = split_text_by_subsentences(text_infilling, ["Preceding Sentence", "Middle Sentence", "Succeeding Sentence"])
